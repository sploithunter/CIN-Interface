...
export class AttentionSystem {
  ...
  private updateTabTitle(): void {
    if (this.queue.length > 0) {
      document.title = `(${this.queue.length}) CIN-Interface`
    } else {
      document.title = 'CIN-Interface'
    }
  }
  ...
  private showBrowserNotification(sessionName: string): void {
    ...
    if (Notification.permission === 'granted') {
      new Notification('CIN-Interface', {
        body: `${sessionName} needs attention`,
        icon: '/favicon.ico',
        tag: 'cin-attention', // Prevents duplicate notifications
      })
