...
async function handleHttpRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (sessionMatch) {
    ...
    if (req.method === 'POST' && action === 'prompt') {
      try {
        const body = await collectRequestBody(req);
        const { prompt, images } = JSON.parse(body) as {
          prompt: string;
          images?: PromptImage[];
        };
        if (!prompt) {
  ...
  if (req.method === 'GET' && filesMatch) {
    ...
    try {
      ...
      res.writeHead(500, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ ok: false, error: `Failed to read directory: ${(e as Error).message}` }));
    }
    return;
  }
  ...
  if (req.method === 'GET' && fileMatch) {
    ...
    try {
      ...
      res.writeHead(500, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ ok: false, error: `Failed to read file: ${(e as Error).message}` }));
    }
    return;
  }
  ...
  if (req.method === 'GET' && treeMatch) {
    ...
    try {
      ...
      res.writeHead(500, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ ok: false, error: `Failed to build tree: ${(e as Error).message}` }));
    }
    return;
  }
