...
export class CINSessionManager extends EventEmitter {
    ...
    config;
    dirty = false;
    constructor(bridgeManager, config) {
        super();
     ...
     */
    async sendPrompt(id, prompt, images) {
        const session = this.getSession(id);
        if (!session) {
        ...
        if (images && images.length > 0 && session.cwd) {
            ...
            for (let i = 0; i < images.length; i++) {
                ...
                try {
                    ...
                    this.debug(`Failed to save image ${filename}: ${e.message}`);
                }
            }
     ...
     */
    findOrCreateSession(agentSessionId, agent, cwd, terminal) {
        const session = this.bridgeManager.findOrCreateSession(agentSessionId, agent, cwd, terminal);
     ...
     */
    getSessionByAgentId(agentSessionId) {
     ...
     */
    mapFlagsToCAB(agent, flags) {
        if (!flags)
            return undefined;
        const cabFlags = {};
        if (agent === 'codex') {
            if (flags.model)
                cabFlags.model = flags.model;
            if (flags.skipPermissions)
                cabFlags['dangerously-bypass-approvals-and-sandbox'] = true;
            else if (flags.fullAuto)
                cabFlags['full-auto'] = true;
            else {
                if (flags.sandbox)
                    cabFlags.sandbox = flags.sandbox;
                if (flags.approval)
                    cabFlags['ask-for-approval'] = flags.approval;
            }
        }
        else {
            // Claude
 ...
 */
export function createCINSessionManager(bridgeManager, config) {
    return new CINSessionManager(bridgeManager, config);
}
//# sourceMappingURL=CINSessionManager.js.map